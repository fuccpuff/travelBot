...
def handle_select_companion_callback(call):
    ...
    if call.data == "finish_selecting_companions":
        if chat_id not in selected_companions or not selected_companions[chat_id]:
            bot.send_message(chat_id, "Вы не выбрали ни одного попутчика.")
        else:
            for companion_id in selected_companions[chat_id]:
            ...
            del selected_companions[chat_id]
    else:
        companion_id = int(call.data.split(":")[-1])
        if chat_id not in selected_companions:
            selected_companions[chat_id] = set()
        selected_companions[chat_id].add(companion_id)
